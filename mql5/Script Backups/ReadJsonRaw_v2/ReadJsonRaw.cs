...
void OnStart()
{
   ...
   if(channels > 0)
   {
      ...
      for(int i = 0; i < channels; i++)
      {
         ...
         string ChannelName = json["channel"][i]["channel_name"].ToStr();

         datetime OpenTime = StringToTime(json["channel"][i]["initial"].ToStr());
         datetime CloseTime = StringToTime(json["channel"][i]["final"].ToStr());

         // input datetime OpenTime = D'2025.12.10 01:00:00';
         // input datetime CloseTime = D'2025.12.11 00:00:00';

         double ChannelHigh = json["channel"][i]["high"].ToDbl();
         double ChannelLow = json["channel"][i]["low"].ToDbl();

         CreateObject(ChannelName, ChannelHigh, ChannelHigh, OpenTime, CloseTime, LineColor, LineWidth);
         CreateObject(ChannelName, ChannelLow, ChannelLow, OpenTime, CloseTime, LineColor, LineWidth);
